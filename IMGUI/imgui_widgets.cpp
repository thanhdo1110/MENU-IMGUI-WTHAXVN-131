...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.selected_rect = ImLerp(it_anim->second.selected_rect, *v ? 1.0f : 0.0f, 0.08f * (2.f - ImGui::GetIO().DeltaTime));

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(48, 48, 52), 9.0f);

    ImVec4 frame_bg_color = ImGui::GetStyleColorVec4(ImGuiCol_FrameBg);
    ImU32 frame_bg_color_u32 = ImGui::GetColorU32(ImVec4(frame_bg_color.x, frame_bg_color.y, frame_bg_color.z, frame_bg_color.w * it_anim->second.selected_rect));

    // Vẽ hình chữ nhật của checkbox với màu nền đã lấy
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), frame_bg_color_u32, 9.0f);

    // Lấy màu cho hình vuông thay thế dấu checkmark
    ImVec4 square_color = ImGui::GetStyleColorVec4(ImGuiCol_CheckMark);
    ImU32 square_color_u32 = ImGui::GetColorU32(ImVec4(square_color.x, square_color.y, square_color.z, square_color.w * it_anim->second.selected_rect));

    // Kích thước và vị trí của hình vuông, tăng chiều dài ra một chút
    float rect_width = square_sz * 0.6f; // Chiều dài tăng thêm
    float rect_height = square_sz * 0.5f; // Chiều cao như cũ
    ImVec2 square_pos_min = ImVec2(
        total_bb.Min.x + (square_sz - rect_width) / 2,
        total_bb.Min.y + (square_sz - rect_height) / 2
    );
    ImVec2 square_pos_max = square_pos_min + ImVec2(rect_width, rect_height);

    // Vẽ hình vuông tăng chiều dài thay cho dấu checkmark
    window->DrawList->AddRectFilled(square_pos_min, square_pos_max, square_color_u32, 9.0f);

    // Tính toán vị trí của nhãn với các khoảng cách điều chỉnh
    ImVec2 label_pos = ImVec2(total_bb.Max.x - label_size.x , total_bb.Min.y + 7);
...
//   const ImVec2 label_size = CalcTextSize(label, NULL, true);
// const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(300, label_size.y + 30)); // Sửa w thành 300px
// const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y + 15 ), total_bb.Max);
...
//     float circle_y = frame_bb.Min.y + (frame_bb.GetHeight() / 2); // Tính toán vị trí y để hình tròn nằm chính giữa slider
// window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x - 1, circle_y), 12.0f, ImColor(1.0f, 1.0f, 1.0f), 30);


//     RenderText(ImVec2(frame_bb.Max.x - 10, total_bb.Min.y - 20), label);
...
//     const ImVec2 label_size = CalcTextSize(label, NULL, true);
//     const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(300, 40)); // Sửa w thành 300px
